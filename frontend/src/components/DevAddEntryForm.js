...
const DevAddEntryForm = ({ onEntryAdded }) => {
    const [date, setDate] = useState('');
    const [name, setName] = useState('');  // This will store the selected name
    const [numberOfBuildings, setNumberOfBuildings] = useState('');
    const [screenshot, setScreenshot] = useState(null);
    const [error, setError] = useState('');
    const [screenshotPreview, setScreenshotPreview] = useState(null);

    // Static list of names for the dropdown
    const nameOptions = [
        'Alice',
        'Bob',
        'Charlie',
    ...
    // Function to capture screenshot (remains unchanged)
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append('name', name);  // Use the selected name
        formData.append('numberOfBuildings', numberOfBuildings);
        formData.append('screenshot', screenshot);

        try {
            await axios.post('http://localhost:5000/api/dev', formData);
            onEntryAdded();
            setDate('');
            setName('');
    ...
    return (
        <div>
            <h1 style={styles.header}>Dev Application</h1>
                ...
                <form onSubmit={handleSubmit} style={styles.form}>
                    ...
                    <div style={styles.formGroup}>
                        ...
                        >
                            ...
                            {nameOptions.map((nameOption, index) => (
...
export default DevAddEntryForm;
